    ...
    if st.button("Display Selected Option"):
        try:
            ...
            if selected_choice == "Similarity Score":
                ...
                predicted_ei = ucf.predict_fitness_score(user_id=selected_user_id,
                                                         feature_column=selected_feature_column,
                                                         top_n=selected_number_users)
                st.markdown(
                    f"#### Predicted {selected_feature_column} for User {selected_user_id} based on similar users: **{predicted_ei:.2f}**")
