...
def streamlit_dashboard(df):
    ...
    with col2:
        ...
        duration = st.slider("Duration", min_value=min_duration, max_value=max_duration,
                             value=(min_duration + max_duration) // 3)
    ...
    if st.button("Display Recommendation"):
        if selected_choice == "Recommended Exercises by Age and BMI":
            ...
            if isinstance(recommended_exercises, list):
                st.write(
                    f"Recommended Exercises for {selected_intensity_category} Intensity Category with <= {duration} min duration:")
                for exercise in recommended_exercises:
                    st.write(f"- {exercise}")
            else:
                st.write(recommended_exercises)
...
def app():
    ...
    # Add a column for Image Path
    # Assuming the images are stored in a specific folder structure
    image_folder = 'C:/Users/ERC/Desktop/free-exercise-db-main/free-exercise-db-main/exercises/'
    df['Image Path'] = image_folder + df['Exercise'].str.replace(" ", "_") + "/0.jpg"  # Update according to your naming convention
