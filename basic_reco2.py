...
def streamlit_dashboard(df):
    ...
    st.title("Exercise Recommendation Dashboard")
    st.write("Columns in the DataFrame:", df.columns)
    st.write(df[['Exercise Types', 'Image Paths', 'Exercise Types.1', 'Image Paths.1']].head())
    ...
    if st.button("Get Exercise Recommendations using KNN"):
